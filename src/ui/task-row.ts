...
import { isDone, type Task } from '../store';
...
export function taskRowHtml(t: Task, opts: { showNobody?: boolean } = {}): string {
  ...
  return `<div class="row" data-id="${esc(t.id)}">
    <button class="chk" data-id="${esc(t.id)}" data-on="${done ? 1 : 0}" aria-pressed="${done}" aria-label="${done ? 'Снять отметку' : 'Отметить сделанной'}">✓</button>
